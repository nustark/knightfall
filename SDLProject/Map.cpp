...
bool Map::IsSolid(glm::vec3 position, float *penetration_x, float *penetration_y) {
    ...
    *penetration_y = 0;
    ...
    int floorTiles[] = {
        0, 32, 35, 50, 80, 81, 96, 97, 98, 99, 112, 113, 114, 115, 128, 129, 130, 131,
        161, 162, 163, 164, 176, 177, 192, 193, 
    };
    size_t floorTilesSize = sizeof(floorTiles) / sizeof(int);
    int *end = floorTiles + floorTilesSize;
    int *result = std::find(floorTiles, end, tile);
    
    if (result != end) {
        return false;
    }
    
    // if (tile == 0 || tile == 32 || tile == 50) return false;
